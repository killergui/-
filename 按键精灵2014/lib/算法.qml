...
Function 提取字串符中所有数字(字符串)
    ...
    rExp.Pattern = "\d+"           //设置过滤模式
    rExp.[Global] = True           //设置全局可用性
    Dim Num, Nums                  //定义变量
    Set Nums = rExp.Execute(字符串)//执行搜索  
    For Each Num In Nums           //遍历匹配集合
        shuzi = shuzi & Num
    next
    Set rExp = Nothing
    提取字串符中所有数字 = shuzi
...
Function 得到字符串中字母的数量(字符串)
    ...
    Dim regEx, Matches, shuliang       //定义变量
    Set regEx = New RegExp             //建立正则表达式
    regEx.Pattern = "([a-z]{1})"       //设置过滤模式
    regEx.IgnoreCase = true            //设置是否区分字符大小写
    regEx.[Global] = True              //设置全局可用性
    Set Matches = regEx.Execute(字符串)//执行搜索   
    shuliang = Matches.count
    Set Matches = Nothing
    得到字符串中字母的数量 = shuliang  
End Function  
